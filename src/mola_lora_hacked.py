...
class LoraMoE_Layer:
    ...
    def reset_lora_parameters_MoE(self, adapter_name):
        ...
        nn.init.kaiming_uniform_(self.router_B['router'].weight, a=math.sqrt(5))
        # -------------------------
        for i in range(self.number_experts):
            adapter_name_moe = adapter_name + '_' + str(i)
...
class Linear_MoE(nn.Linear, LoraMoE_Layer):
    ...
    # 与merge方法相对，用于撤销先前合并的权重修改。
    # 如果适配器激活且已合并，从原始权重中减去之前添加的修改部分。
    # 同样通过merged标志位控制。
    ...
    #     # 根据专家掩码找出当前专家负责的输入。
    #     # 如果当前专家没有输入，则跳过。
    #     # 从 x 中提取当前专家负责的输入，并通过对应的LoRA层 (lora_A 和 lora_B) 进行处理，得到专家的输出。
    #     # 将输出结果乘以路由权重和缩放因子，得到最终的隐藏状态。
    #     # 将这些隐藏状态加和到 final_hidden_states 中。
    #     final_hidden_states = final_hidden_states.reshape(batch_size, sequence_length, self.out_features)
    #     result += final_hidden_states
    #     # 将 final_hidden_states 重新整形为原始输入 x 的形状，并加到原始线性变换结果 result 上。
    #     # 将结果转换回输入的原始数据类型。
    #
    ...
    def forward(self, x: torch.Tensor):
        ...
        router_logits_B = self.router_B['router'](x)
        a = 0.5
        router_logits = a * router_logits_A + (1 - a) * router_logits_B
