...
##newmodified
from src.mola_peft_model_hacked import (
    PeftModel,
    PeftModelForCausalLM,
    PeftModelForQuestionAnswering,
    PeftModelForSeq2SeqLM,
...
def get_peft_model(model: PreTrainedModel, peft_config: PeftConfig, adapter_name: str = "default",
    ...
    # 将模型的配置通过to_dict()方法（如果存在）转换为字典格式，否则直接使用配置对象。这是为了能够更灵活地处理模型配置。
    # 将模型名称或路径赋值给peft_config.base_model_name_or_path属性，以便配置中能够引用原始模型。
    ...
    # 检查配置类型和任务类型是否适合直接创建PeftModel：
    # 如果peft_config.task_type不在MODEL_TYPE_TO_PEFT_MODEL_MAPPING映射的键中，并且配置不是PromptLearningConfig类型，则直接使用
    # PeftModel类构造模型实例并返回。如果peft_config是PromptLearningConfig类型，则调用
    # _prepare_prompt_learning_config函数来准备或调整提示学习配置。
    ...
    # 使用 MODEL_TYPE_TO_PEFT_MODEL_MAPPING 映射中对应的函数根据任务类型、配置、适配器名称、专家数量和 top_k 选择创建并返回一个 PeftModel 实例。
